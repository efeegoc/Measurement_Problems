...
# bombabomba.com şimdi sizden bu A/B testinin sonuçlarını analiz etmemizi bekliyor.Bombabomba.com için
...
# ab_testing_data.xlsx adlı kontrol ve test grubu verilerinden oluşan veri setini okutalım. Kontrol ve test grubu verilerini ayrı değişkenlere atayalım.
...
# Kontrol ve test grubu verilerini analiz edelim.
...
# Elde edilen p-valuedeğerlerini yorumlayalım.

test_stat, pvalue = shapiro(df.loc[df["group"] == "control", "Purchase"])
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
# p-value=0.5891
# HO reddedilemez. Control grubunun değerleri normal dağılım varsayımını sağlamaktadır.
# EĞER NORMAL DAĞILIM VARSAYIMI SAĞLANMASAYDI VARYANS HOMOJENLİĞİNE BAKMADAN NON PARAMETRİC TESTE GEÇİCEKTİK
...
# Adım 1: Hangi testi kullandığımız, sebeplerini belirtelim.

# İlk önce iki grubada normallik testini uyguladık . İki grubun da normal dağılıma uygun olduğu gözlemlendiğinden
# ikinci varsayıma gerçilerek varyans homojenliği incelenmiştir .Varyanslar homojen çıktığından "Bağımsız İki Örneklem T Testi" Uygulanmıştır
# Uygulama sonucunda p değerinin 0.05 ten büyük olduğu gözlemlenmiştir. Ve H0 Hipotezi reddedilmiştir


# Adım 2: Elde ettiğimiz test sonuçlarına göre müşteriye tavsiyede bulunalım.

# Satın alma anlamında anlamlı bir fark olmadığından müşteri iki yöntemden birini seçebilir fakat burada diğer istatistiklerdeeki
# farklılıklarda önem arz edecektir . Tıklanma , etkileşim , kazanç ve dönüşüm oranlarındaki farklılıklar değerlendirilip hangi
# oranın daha düşük olduğu tespit edilip CTR(Click Through Rate] oranına bakılabilir .
